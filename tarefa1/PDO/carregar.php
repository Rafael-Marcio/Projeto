...
try {
    ...
    if ($result) {
        ...
        while ($linha = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            if($comprado == 1){
                $compras .= "<tr style='text-decoration: line-through;'> <td>" . $id . "</td><td>" . $produto . "</td><td>" . $quantidade . "</td><td>" . $marca . "</td><td style='border: transparent;'><button style='background-color: red; border-radius: 10px;' onclick='removerElemento(" . $linha["id"] . ")'>Excluir</button></td><td style='border: transparent;'><button style='background-color: yellow; border-radius: 10px;' onclick='atualizarElemento(" . $linha["id"] . ")'>Editar</button></td><td style='border: transparent;'><button style='background-color: brown; border-radius: 10px;' onclick='Descomprar( " . $linha["id"] . ")'>Comprado</button></td></tr>";;
            }else{
                $comprados .= "<tr> <td>" . $id . "</td><td>" . $produto . "</td><td>" . $quantidade . "</td><td>" . $marca . "</td><td style='border: transparent;'><button style='background-color: red; border-radius: 10px;' onclick='removerElemento(" . $linha["id"] . ")'>Excluir</button></td><td style='border: transparent;'><button style='background-color: yellow; border-radius: 10px;' onclick='atualizarElemento(" . $linha["id"] . ")'>Editar</button></td><td style='border: transparent;'><button style='background-color: greenyellow; border-radius: 10px;' onclick='Comprado(" . $linha["id"] . ")'>Comprar</button></td></tr>";
